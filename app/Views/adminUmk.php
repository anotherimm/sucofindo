...
<body class="bg-gray-100">
    ...
    <div class="pt-28 pl-4 md:pl-48 pr-4 md:pr-10">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-xl font-bold mb-4">UANG MUKA</h2>

            <form id="prosesUmkForm" action="<?= base_url('adminUmk/saveUmk') ?>" method="post">
                <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="tanggalUmk" class="block mb-2">Tanggal Uang Muka:</label>
                        <input type="date" id="tanggalUmk" name="tanggal_umk" value="<?= $form_data['tanggal_umk'] ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                    <div>
                        <label for="hargaUmk" class="block mb-2">Harga Uang Muka:</label>
                        <input type="text" id="hargaUmk" name="harga_umk" value="<?= esc(old('hargaUmk', $form_data['harga_umk'])) ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                    <div>
                        <label for="vendorUmk" class="block mb-2">Nama Vendor:</label>
                        <input type="text" id="VendorUmk" name="vendor_umk" value="<?= $form_data['vendor_umk'] ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                </div>
                <div class="flex flex-col md:flex-row justify-end mt-4 space-y-4 md:space-y-0 md:space-x-4">
                    <a href="<?= base_url('adminUmk/resetUmk?tambahdata_id=' . $form_data['tambahdata_id']) ?>" class="w-full md:w-auto bg-gray-200 text-gray-700 py-2 px-4 rounded-md text-center">Reset</a>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    const input = document.getElementById('hargaUmk');
                    ...
                    document.getElementById('prosesUmkForm').addEventListener('submit', function(event) {
                        // Mencegah pengiriman form default
                        event.preventDefault();

                        let nilaiInput = document.getElementById('hargaUmk');
