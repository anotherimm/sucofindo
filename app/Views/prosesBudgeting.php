...
<body class="bg-gray-100">
    ...
    <div class="pt-28 pl-4 md:pl-48 pr-4 md:pr-10">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-xl font-bold mb-4">Proses Budgeting</h2>

            <form id="prosesBudgetForm" action="<?= base_url('prosesBudgeting/saveBudget') ?>" method="post">
                <?= csrf_field() ?>
                <input type="hidden" name="tambahdata_id" value="<?= $form_data['tambahdata_id'] ?>">

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="tanggalMasukBudget" class="block mb-2">Tanggal Masuk Budget :</label>
                        <input type="date" id="tanggalMasukBudget" name="tanggalMasukBudget" value="<?= old('tanggalMasukBudget', $form_data['tanggal_masuk_budget']) ?>" class="w-full p-2 border border-gray-300 rounded-md" <?= $is_locked ? 'disabled' : '' ?> required>
                    </div>

                    <div>
                        <label for="tanggalDiterimaSetelahBudgeting" class="block mb-2">Tanggal Diterima Setelah Budgeting :</label>
                        <input type="date" id="tanggalDiterimaSetelahBudgeting" name="tanggalDiterimaSetelahBudgeting" value="<?= old('tanggalDiterimaSetelahBudgeting', $form_data['tanggal_diterima_setelah_budgeting']) ?>" class="w-full p-2 border border-gray-300 rounded-md" <?= $is_locked ? 'disabled' : '' ?> required>
                    </div>

                    <div class="mb-4">
                        <label for="penerimaDokumenBudget" class="block mb-2">Penerima Dokumen Budget</label>
                        <select name="penerimaDokumenBudget" id="penerimaDokumenBudget" class="w-full p-2 border border-gray-300 rounded-md" <?= $is_locked ? 'disabled' : '' ?> required>
                            <option value="">Pilih Penerima</option>
                            <?php foreach ($penerima_options as $penerima) : ?>
                                <option value="<?= $penerima['id'] ?>" <?= ($form_data['penerima_dokumen_budget'] === $penerima['nama_penerima']) ? 'selected' : '' ?>>
                ...
                <div class="flex flex-col md:flex-row justify-end mt-4 space-y-4 md:space-y-0 md:space-x-4">
                    <a href="<?= base_url('prosesBudgeting/resetBudget?tambahdata_id=' . $form_data['tambahdata_id']) ?>" class="w-full md:w-auto bg-gray-200 text-gray-700 py-2 px-4 rounded-md text-center">Reset</a>
            ...
            <script>
                ...
                document.getElementById('prosesBudgetForm').addEventListener('submit', function(event) {
