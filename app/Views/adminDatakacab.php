...
<body class="bg-gray-100">
        ...
        <div class="container mx-auto">
            <div class="flex justify-between items-center mb-6">
                <div class="flex space-x-2">
                    <a href="/adminDatakepala" class="bg-gray-400 text-white font-bold border-none py-2 px-4 rounded-lg cursor-pointer">Kabid</a>
                    <a href="/adminDatakacab" class="bg-blue-600 text-white font-bold border-none py-2 px-4 rounded-lg cursor-pointer">Kacab</a>
                </div>
                <div class="flex items-center">
                    <input type="text" id="search" class="border border-gray-300 rounded py-1 px-2 w-80" placeholder="Cari Kepala Cabang" oninput="searchTable()">
                </div>
            </div>
                ...
                <table id="tableData" class="min-w-full bg-white shadow-md rounded-lg">
                    <thead>
                        <tr class="bg-blue-600 text-white text-center">
                            ...
                            <th class="w-2 border p-3 ">Nama Kepala Cabang</th>
                            <th class="w-3 border p-3 ">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!-- Menampilkan data dari controller -->
                        <?php foreach ($kacabs as $index => $kacab) : ?>
                            <tr>
                                <td class="border p-3 text-center"><?= $index + 1 ?></td>
                                <td class="border p-3"><?= esc($kacab['nama_kacab']) ?></td>
                                <td class="border p-3 text-center">
                                    <button class="bg-yellow-500 text-white border-none py-1 px-3 rounded cursor-pointer edit-button" data-id="<?= $kacab['id'] ?>" data-nama-kacab="<?= esc($kacab['nama_kacab']) ?>">Edit</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-lg p-6 w-1/2">
            <div class="flex justify-between items-center border-b pb-3">
                <h3 class="text-lg font-bold">Edit Kacab</h3>
                <button class="text-black" onclick="closeEditModal()">✖</button>
            </div>
            <div class="mt-4">
                <form id="editForm" action="<?= base_url('adminDatakacab/updateKacab') ?>" method="post">
                    <input type="hidden" id="editId" name="id">
                    <div class="mb-4">
                        <label for="namaKacab" class="block text-sm font-medium text-gray-700">Nama Kacab:</label>
                        <input type="text" id="namaKacab" name="namaKacab" class="mt-1 block w-full border border-gray-300 rounded py-2 px-3 shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>
    ...
    <script>
        ...
        function showEditModal(id, namaKacab) {
            document.getElementById('editId').value = id;
            document.getElementById('namaKacab').value = '';
        ...
        editButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                var namaKacab = this.getAttribute('data-nama-kacab');
                showEditModal(id, namaKacab);
            });
        });
        ...
        document.getElementById('editForm').addEventListener('submit', function(event) {
            ...
            var namaKacab = document.getElementById('namaKacab').value;

            var formData = new FormData();
            formData.append('id', id);
            formData.append('namaKacab', namaKacab);

            fetch('<?= base_url("adminDatakacab/updateKacab") ?>', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        row.querySelectorAll('td')[1].textContent = namaKacab;
