...
<body class="bg-gray-100">
    ...
    <div class="pt-28 pl-4 md:pl-48 pr-4 md:pr-10">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-xl font-bold mb-4">KONTRAK</h2>

            <form id="prosesKontrakForm" action="<?= base_url('adminKontrak/saveKontrak') ?>" method="post" class="<?= $is_locked ? 'form-disabled' : '' ?>">
                <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="nomorKontrak" class="block mb-2">Nomor Kontrak:</label>
                        <input type="text" id="nomorKontrak" name="nomor_kontrak" value="<?= $form_data['nomor_kontrak'] ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                    <div>
                        <label for="tanggalKontrak" class="block mb-2">Tanggal Kontrak:</label>
                        <input type="date" id="tanggalKontrak" name="tanggal_kontrak" value="<?= $form_data['tanggal_kontrak'] ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                    <div>
                        <label for="vendorPemenang" class="block mb-2">Vendor Pemenang:</label>
                        <input type="text" id="vendorPemenang" name="vendor_pemenang" value="<?= $form_data['vendor_pemenang'] ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                    <div>
                        <label for="hargaKontrak" class="block mb-2">Harga:</label>
                        <input type="text" id="hargaKontrak" name="harga_kontrak" value="<?= esc(old('hargaKontrak', $form_data['harga_kontrak'])) ?>" class="w-full p-2 border border-gray-300 rounded-md" required <?= $is_locked ? 'disabled' : '' ?>>
                    </div>
                </div>
                <div class="flex flex-col md:flex-row justify-end mt-4 space-y-4 md:space-y-0 md:space-x-4">
                    <a href="<?= base_url('adminKontrak/resetKontrak?tambahdata_id=' . $form_data['tambahdata_id']) ?>" class="w-full md:w-auto bg-gray-200 text-gray-700 py-2 px-4 rounded-md text-center">Reset</a>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    const input = document.getElementById('hargaKontrak');
                    ...
                    document.getElementById('prosesKontrakForm').addEventListener('submit', function(event) {
                        // Mencegah pengiriman form default
                        event.preventDefault();

                        let nilaiInput = document.getElementById('hargaKontrak');
